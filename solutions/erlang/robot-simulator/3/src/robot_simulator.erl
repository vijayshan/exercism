...
handle_call(advance, _From, RobotState) ->
    Position = maps:get(position, RobotState),
    case maps:get(direction, RobotState) of
        north ->
            {reply,
             ok,
             maps:update(position, {element(1, Position), element(2, Position) + 1}, RobotState)};
        east ->
            {reply,
             ok,
             maps:update(position, {element(1, Position) + 1, element(2, Position)}, RobotState)};
        south ->
            {reply,
             ok,
             maps:update(position, {element(1, Position), element(2, Position) - 1}, RobotState)};
