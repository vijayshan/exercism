  defmodule RomanNumerals do
    ...
    def numeral(number) do
      list=case number do
        num when num>=1000 ->
          Enum.concat(Enum.map(1..trunc(num/1000),fn _->'M' end), roman_numerals(rem(num,1000), @digits, 100))
        num ->
    ...
    def roman_numerals(number, rep_list, place)do
      [tens, fives, ones|_] = rep_list
      [_,_|tail] = rep_list
      # the mapping is based in simple rules
      # the special cases are mapped here
      ret_list = case trunc(number/place) do
          # 0
          digit when digit==0 ->
            []
          # less 1,2,3
